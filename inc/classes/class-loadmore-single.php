	...
	*/
	function single_post_load_more_container() {
		...
		<div class="single-post-loadmore-wrap">
			...
			<div class="text-center mb-5 mt-5">
				...
				>
					<span><?php esc_html_e( 'Load More Stories', 'deail365' ); ?></span>
				</button>
				<span id="single-loading-text"
				      class="mt-1 hidden"><?php esc_html_e( 'Loading...', 'deail365' ); ?></span>
			</div>
		</div>
		<?php
	}
	...
	public function get_single_load_more_query( $page_no, $single_post_id ) {
		...
		$args = [
			...
			'worthio_single_load_more_query' => true,
		];
	...
	function posts_where( $where, $query ) {
		...
		$worthio_single_load_more = $query->get( 'worthio_single_load_more_query' );

		// If the query does not have our custom param 'starting_post_id' return default where clause.
		if ( empty( $start ) || empty( $worthio_single_load_more ) ) {
			return $where;
		}

		$where .= $wpdb->prepare( " AND {$wpdb->posts}.ID < %d", absint( $start ) );
